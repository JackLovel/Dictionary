...
void Dict::queryWord(const QString &word)
{
    ...
    request.setUrl(QUrl("https://www.shanbay.com/api/v1/bdc/search/?word=" + word));
...
//    data = new ResponedData(name, ukPron, usPron,chinese, ukAudio, ukAudio, message);

    emit signalQueryFinished(name, ukPron, usPron,chinese, ukAudio, ukAudio, message);
}
